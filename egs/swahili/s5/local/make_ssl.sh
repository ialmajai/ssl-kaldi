...
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
  cat >&2 <<EOF
Usage: $0 [options] <data-dir> [<log-dir> [<ssl-dir>] ]
 e.g.: $0 data/train
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mhubert_${name}.JOB.log \
    ...
    python local/compute_ssl_feats.py --layer $layer --model $ssl_model \
  ...
  $cmd JOB=1:$nj $logdir/make_ssl_${name}.JOB.log \
    python local/compute_ssl_feats.py --model $ssl_model --layer $layer $write_utt2dur_opt \
