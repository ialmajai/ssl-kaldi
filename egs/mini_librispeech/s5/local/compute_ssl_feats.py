...
# ssl_extractor = SSLExtractor("facebook/mms-1b")  

# scp_path = args.input 
# output_ark = args.output

# if scp_path == 'ark:-':
#     reader = kaldiio.read_mat_ark('ark:-')  # reads ark:- from stdin
#     with kaldiio.WriteHelper('ark:-') as writer:
#         for utt, wav in reader:
#             features, duration = ssl_extractor.extract(utt_id, target_layer=-1)
#             utt2dur_data[utt_id] = np.array([duration], dtype=np.float32)

#             writer(utt_id, features)

# else:    
#     with ReadHelper(scp_path) as reader:
#         utt2dur_data = {}
#         with WriteHelper(output_ark) as writer:
#             for utt_id, (sample_rate, waveform) in reader:
...
# # Write utt2dur if specified
# if args.write_utt2dur:
#     if args.write_utt2dur.startswith("ark,t:"):
#         utt2dur_writer_path = args.write_utt2dur[6:]
#     elif args.write_utt2dur.startswith("ark:"):
#         utt2dur_writer_path = args.write_utt2dur[4:]
        
#     with open(utt2dur_writer_path, "w") as f:
#         for utt_id, dur in utt2dur_data.items():
#             f.write(f"{utt_id} {dur.item():.3f}\n")
