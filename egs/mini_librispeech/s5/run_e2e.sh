...
if [ $stage -le 4 ]; then
    ...
    local/make_hubert.sh --cmd "$train_cmd" --nj 4 \
	    --layer $encoder_layer data/${trainset}_spe2e_raw exp/make_hubert/${trainset}_spe2e_raw $featdir
    steps/compute_cmvn_stats.sh data/${trainset}_spe2e_raw exp/make_hubert/${trainset}_spe2e_raw $featdir   
fi
